...
- 비디오를 처리하는 프론트엔드 애플리케이션이 있다고 가정하자. 이 처리를 프론트엔드에서 직접 처리하면 서버가 느려질 수 있는 위험이 존재한다.

- 이를 분리해서 처리하기 위해 프론트엔드 애플리케이션은 `SQS`의 대기열로 메세지를 전송한다.

- 그리고 백엔드 애플리케이션은 해당 메세지를 수신하여 실제 비디오 작업을 수행하고 `S3`에 사입한다.
