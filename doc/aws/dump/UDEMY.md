...
→ `Amazon GuardDuty`를 사용해 S3에 저장된 데이터에 대한 악의적인 시도를 모니터링하고, `Amazon Macie`를 사용해 `S3`에 저장된 민감 데이터를 식별한다.
...
→ 아니다. `API` 요청은 가장 가까운 `CloudFront` 엣지 로케이션으로 라우팅되며, 이는 지연 시간을 향상시킨다. `API Gateway`는 여전히 하나의 `AWS` 리전에 존재한다.
...
- 부서 별로 새 사용자에게 권한을 부여하는 가장 안전한 방법은 무엇입니까?
→ 부서별로 `IAM` 그룹을 설정한다. `IAM` 정책을 생성하고 `IAM` 정책을 각 `IAM` 그룹에 연결한다.
