...
→ 인스턴스를 부팅할 때 자동화하고 싶은 작업(업데이트, 소프트웨어 설치, 일반적인 파일을 다운로드 그 외의 생각하는 모든 것)들을 설정할 수 있다.
...
~~~
1. 조직 전반의 최신 데이터 수집
2. 이해하기 쉬운 양식으로 데이터 표시(표, 그래프 등)
3. 조직 내 직원들에게 시의적절하게 데이터 전달
~~~
...
→ 보안 그룹은 지역과 `VPC`의 결합으로 통제되어 있다. 그래서 지역을 변경하면 새 보안그룹을 생성하거나 다른 `VPC`를 생성해야 한다.

- Does live “outside” the EC2 – if traffic is blocked the EC2 instance won’t see it
→ 보안 그룹은 `EC2` 외부에 존재한다. 그래서 트래픽이 차단되면 `EC2` 인스턴스는 확인할 수 없다. `EC2`에서 실행하는 애플리케이션이 아니라 `EC2` 외부의 방화벽인 것이다.

- It’s good to maintain one separate security group for SSH access
→ `SSH` 액세스를 위해 하나의 별도 보안 그룹을 유지하는 것이 좋다. 보통 `SSH` 액세스는 가장 복잡하므로 별도 보안 그룹이 잘 완료되었는지 확인해야 한다.
 
- If your application is not accessible (time out), then it’s a security group issue
→ 타임아웃으로 `EC2`에 접근할 수 없으면 이것은 보안 그룹의 문제이다.
...
`EC2` 인스턴스가 있고, 보안 그룹 1이라는 보안 그룹이 있다. 그리고, 기본적인 인바인드 규칙은 보안 그룹 1의 인바운드를 보안 그룹 2에 허용하는 것이다.
...
→ `Mac`, `Linux`, `Windows 10`에서는 `.pem` 형식을 사용하지만, `Windows 10` 미만의 윈도우 버전에서는 `.ppk`를 `PuTTY`와 함께 사용한다. `PuTTY`는 `SSH(Secure Shell)` 접속을 위한 클라이언트이다.
`EC2` 서버에 접속하기 위한 다른 방법으로 `EC2 Instance Connect`도 있는데, 이를 사용하면 터미널이나 클라이언트 프로그램이 아닌 웹 브라우저로 `EC2 인스턴스`에 연결할 수 있다.
좋은 점은 모든 버전의 `Mac`, `Linux`, 그리고 `Windows`에서 사용이 가능하다는 것이다.
...
- 접속 후 나가기 위해서는 `exit`를 입력하거나, `ctrl + z`를 누르면 된다.
...
- 자격 증명이 필요한 서비스는 `Access Key`를 요구한다. 하지만, 이는 서버상에 절대로 입력하면 안 된다. 다른 유저들이 인스턴스에 입력된 자격 증명 정보를 회수할 수 있게 되기 때문이다.
~~~ shell script
$ aws iam list-users
~~~
...
→ 리눅스 또는 윈도우는 초 단위로 요금을 지불해야 하며 다른 모든 운영체제의 경우에는 1시간 단위로 청구가 이루어지므로 비용 예측이 쉽다.
또, 비용이 가장 많이 들지만 바로 지불할 금액은 없고 장기적인 약정도 필요없다. 
그렇기 때문에 단기적이고 중단 없는 워크로드가 필요할 때 또는 애플리케이션의 거동을 예측할 수 없을 때 가장 추천할 수 있다.

- Reserved Instance (1 & 3 years)
→ 예약 인스턴스를 사용하기 위해서 사용자는 특정한 인스턴스의 속성을 예약해야 하며, 속성에는 인스턴스의 타입, 리전, `Tenancy`, 운영체제 등이 포함된다. 
이 외에도 예약 기간을 1년이나 3년으로 지정해서 할인을 더 받을 수 있으며 전부 선결제면 최대 할인을 받을 수 있다.
그렇기 때문에 사용량이 일정한 애플리케이션이라면 예약 인스턴스를 사용하는 것이 가장 효율적이다.
...
그리고 사용량이 한도를 넘으면 절약 플랜은 온디맨드 가격으로 청구를 하게 된다.

- Spot Instances – short workloads, cheap, can lose instances (less reliable)
→ 온디맨드와 비교해 90% 가량의 할인을 받을 수 있는 플랜이다. 
...
→ 용량 예약을 이용하면 원하는 기간 동안 특정한 `AZ, Availability Zone`에 용량을 예약할 수 있다. 그리고 필요할 때마다 그 용량에 접근할 수 있다.
기간 약정은 없고, 언제라도 용량을 예약하고 취소할 수 있다. 청구 할인도 없는 용량을 예약하는 것이 유일한 목적인 플랜이다.
그래서, 만약 청구 할인을 받기 위해서라면 예약 인스턴스와 결합하거나 절약 플랜과 결합해야 한다.
